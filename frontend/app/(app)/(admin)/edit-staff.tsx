...
import WorkingHoursEditor, { WorkingHours } from "./components/WorkingHoursEditor";
...
const useStaffMember = (staffId?: string) => {
    return useQuery<User | null, Error>({
        ...
        queryFn: () => staffId ? userService.getUserById(staffId) : null,
...
export default function EditStaffScreen() {
  ...
  const { mutate: updateUser, isPending: isUpdating } = useMutation<User, Error, Partial<User>>({
      mutationFn: (updates) => userService.updateUser(staffId!, updates),
      onSuccess: () => {
  ...
  const { mutate: createStaff, isPending: isCreating } = useMutation<User, Error, Omit<User, 'id'>>({
      mutationFn: (newData) => userService.createStaff(newData as any),
      onSuccess: () => {
  ...
  const { mutate: exchangeCode, isPending: isExchangingCode } = useMutation<any, Error, { code: string; staffId: string; }>({ 
      mutationFn: (vars) => {
          const handleGoogleAuthCallback = httpsCallable(getFunctions(), 'handleGoogleAuthCallback');
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <Text style={styles.header}>{isEditMode ? "Edit Staff Member" : "Add New Staff"}</Text>
        
        <View style={styles.formSection}>
            <Text style={styles.sectionTitle}>Basic Information</Text>
            <TextInput style={styles.input} placeholder="First Name" value={firstName} onChangeText={setFirstName} />
            <TextInput style={styles.input} placeholder="Last Name" value={lastName} onChangeText={setLastName} />
            <TextInput style={[styles.input, isEditMode && styles.disabledInput]} placeholder="Email Address" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" editable={!isEditMode} />
            <TextInput style={styles.input} placeholder="Phone Number" value={phone} onChangeText={setPhone} keyboardType="phone-pad" />
            <TextInput style={styles.input} multiline placeholder="Bio" value={bio} onChangeText={setBio} />
        </View>

        <StaffServiceSelector selectedServiceIds={serviceIds} onToggleService={handleToggleService} />

        <WorkingHoursEditor workingHours={workingHours} onUpdate={handleUpdateWorkingHours} />
      ...
      <View style={styles.footer}>
        <Button title={isEditMode ? "Save Changes" : "Create Staff Member"} onPress={handleSave} disabled={isUpdating} />
      </View>
    </SafeAreaView>
  );
}
