...
export default function SeedingPage() {
  ...
  const [seedingResults, setSeedingResults] = useState<SeedingResult | null>(null);
  ...
  const handleFullSeed = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('✅ Users created:', result.users?.map(u => `${u.email} (${u.role})`));
      } else {
      ...
      setSeedingResults({ success: false, error: error.message || 'Unknown error occurred' });
    } finally {
      setIsSeeding(false);
    }
  };
  ...
  const handleSeedUsers = async () => {
    ...
    try {
      ...
      console.log('✅ Created users:', users.map(u => `${u.email} (${u.role})`));
    } catch (error: any) {
      ...
      <View style={{ padding: 20 }}>
        <View style={{
          ...
          <View style={{
            ...
            <Text style={{ color: '#856404', fontWeight: 'bold', marginBottom: 5 }}>
              ⚠️ Warning:
            </Text>
            <Text style={{ color: '#856404', fontSize: 14 }}>
              This will create real data in your Firebase project. Make sure you're connected to your development environment.
            </Text>
          </View>
          ...
          <View style={{ marginBottom: 20 }}>
            ...
            <Text style={{ fontSize: 13, color: '#888', marginLeft: 20, marginBottom: 3 }}>
              • Admin: admin@bookingsystem.com / admin123
            </Text>
            <Text style={{ fontSize: 13, color: '#888', marginLeft: 20, marginBottom: 3 }}>
              • Staff: sarah@bookingsystem.com / stylist123
            </Text>
            <Text style={{ fontSize: 13, color: '#888', marginLeft: 20, marginBottom: 8 }}>
              • Customer: customer@example.com / customer123
            </Text>
            
            <Text style={{ fontSize: 14, color: '#666', marginBottom: 5 }}>
              ✂️ <Text style={{ fontWeight: 'bold' }}>5 Services:</Text> Haircuts, coloring, styling, treatments
            </Text>
            <Text style={{ fontSize: 14, color: '#666', marginBottom: 5 }}>
              🕒 <Text style={{ fontWeight: 'bold' }}>Business Hours:</Text> Standard schedules for staff
            </Text>
          </View>
          ...
          >
            <Text style={{ color: 'white', fontWeight: 'bold', fontSize: 16 }}>
              {isSeeding ? '🌱 Seeding Database...' : '🚀 Seed Complete Database'}
            </Text>
          </TouchableOpacity>
            ...
            <View style={{
              ...
              <Text style={{
                ...
                {seedingResults.success 
                  ? `Created ${seedingResults.users?.length || 0} users and ${seedingResults.services?.length || 0} services`
