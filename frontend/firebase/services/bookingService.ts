...
class BookingService {
  ...
  async createBooking(bookingData: CreateBookingData): Promise<Booking> {
    try {
      ...
      const docRef = await addDoc(collection(db, this.bookingsCollection), bookingDoc);
  ...
  async updateBooking(id: string, updates: Partial<Booking>): Promise<Booking> {
    try {
      ...
      if (updateData.appointmentDate && updateData.appointmentDate instanceof Date) {
  ...
  async getAvailableTimeSlots(
    ...
    try {
      ...
      const staffBookings = existingBookings.filter(
        booking => booking.staffMemberId === staffMemberId && 
      ...
      return allSlots.filter(slot => {
        ...
        return !staffBookings.some(booking => 
          this.timesOverlap(slot, slotEndTime, booking.startTime, booking.endTime)
        );
      });
    } catch (error: any) {
